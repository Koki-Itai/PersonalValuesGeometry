...
def apply_ica(embeddings, n_components=10):
...
def show_histogram_LOO(
    ...
    # Save figure
    if is_ica:
        save_dir = os.path.join(save_dir, "ica")
...
if __name__ == "__main__":
    ...
    reduced_random_diff_embeddings = apply_ica(random_diff_embeddings, n_components=10)
    reduced_random_inner_product_LOO = compute_inner_product_LOO_with_ica(
        reduced_random_diff_embeddings
    )

    # Counterfactual pairの生成
    all_inner_product_LOO = []
    all_inner_product_LOO_with_ica = []
    # all_concept_vectors = []
    ...
    for _, filename in enumerate(filenames):
        ...
        # ICA
        reduced_diff_embeddings = apply_ica(
            diff_embeddings, n_components=10
        )  # shape: (num_sample, n_components)
        reduced_inner_product_LOO = compute_inner_product_LOO_with_ica(
            reduced_diff_embeddings
        )
        all_inner_product_LOO_with_ica.append(reduced_inner_product_LOO)
    ...
    reduced_inner_product_matrix_path = os.path.join(
        inner_product_matrix_path, "ica/reduced_concept_diff_embeddings.npy"
    )
    ...
    # Visualize LOO histograms with ica reduced embeddings
    show_histogram_LOO(
        all_inner_product_LOO=all_inner_product_LOO_with_ica,
