...
def apply_ica(matrix: torch.Tensor, n_components: int = 2) -> torch.Tensor:
    pca_input = ICAInput(matrix=matrix, n_components=n_components)
    matrix_np = pca_input.matrix.detach().cpu().numpy()

    ica = ICA(n_components=pca_input.n_components)
    reduced_vectors = ica.fit_transform(matrix_np)
...
# reduced_x = apply_ica(x, n_components=3)
